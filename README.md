...
[![coverage 98.96%](https://img.shields.io/badge/coverage-98.96%25-brightgreen)](hk.pkl)
...
Part of that superset is not a rule but a *section*: `§F FEDERATION` and `§N NAV`, which let one spec span a directory tree instead of sitting alone at the root. Those are additions to the format itself, so they are written down where the format is — [`FORMAT-EXTENSIONS.md`](FORMAT-EXTENSIONS.md), beside the vendored file, generated by `mth extensions` and frozen by a test. It is written for a reader who does **not** have this tool: copy it next to your own `FORMAT.md` and the sections are adoptable by hand. Every extension section is optional, and a spec carrying none of them is unaffected.
