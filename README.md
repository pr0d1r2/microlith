...
[![coverage 99.45%](https://img.shields.io/badge/coverage-99.45%25-brightgreen)](hk.pkl)
...
Part of that superset is not a rule at all but an addition to the *format*, and those come in two shapes. A **section**: `§F FEDERATION` and `§N NAV`, which let one spec span a directory tree instead of sitting alone at the root. A **marker**: `[superseded by V9]`, which retires a rule without deleting it — deletion would free the id for reuse and strand every citation that still names it.

Both are written down where the format is — [`FORMAT-EXTENSIONS.md`](FORMAT-EXTENSIONS.md), beside the vendored file, generated by `mth extensions` and frozen by a test. It is written for a reader who does **not** have this tool: copy it next to your own `FORMAT.md` and the extensions are adoptable by hand. Every one of them is optional, and a spec carrying none is unaffected.
...
The spec changes first, then the code -- [`CONTRIBUTING.md`](docs/CONTRIBUTING.md) explains the loop and how to get set up. [`INTEGRATION.md`](docs/INTEGRATION.md) is the guardrails: what runs at commit, at push and on CI, and why in that order. [`AGENTS.md`](AGENTS.md) is the deeper working guide, for humans and agents alike.
