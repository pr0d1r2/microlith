{
  # cavespec's pinned toolchain. Lives at the repo ROOT, which is not
  ...
  description = "cavespec -- the cavekit SPEC format, enforced (dev shell)";
  ...
  outputs =
    ...
    let
      ...
      # cavespec formats its own SPEC.md (V24), so the dev shell must PROVIDE
      # `cavespec`, not merely the toolchain to build it.
      #
      ...
      cavespecShim =
        ...
        pkgs.writeShellScriptBin "cavespec" ''
          ...
          if [ -z "$manifest" ] || [ ! -f "$manifest" ]; then
            echo "cavespec(shim): CAVESPEC_MANIFEST unset or missing -- re-enter the dev shell from the repo root" >&2
            exit 2
          fi
          profile=""
          [ "''${CAVESPEC_PROFILE:-debug}" = "release" ] && profile="--release"
          exec cargo run --quiet $profile \
            --manifest-path "$manifest" --bin cavespec -- "$@"
        '';
      ...
      # and CI is the gate of record either way (V22).
      #
      ...
      gitHook =
        pkgs: name:
        pkgs.writeText "cavespec-${name}-hook" ''
          #!/bin/sh
          # Generated by cavespec's dev shell (flake.nix), rewritten on every
      ...
      # lets nix vendor crates offline. cavespec has zero dependencies, so
      ...
      cavespecPkg =
        ...
        pkgs.rustPlatform.buildRustPackage {
          ...
          meta = {
            ...
            homepage = "https://github.com/pr0d1r2/cavespec";
            license = pkgs.lib.licenses.mit;
            mainProgram = "cavespec";
          };
        };
    in
    {
      packages = forAll (pkgs: {
        default = cavespecPkg pkgs;
      });
        ...
        default = pkgs.mkShell {
          ...
          shellHook = ''
            if [ -f "$PWD/Cargo.toml" ]; then
              export CAVESPEC_MANIFEST="$PWD/Cargo.toml"
            else
              echo "cavespec(shell): no Cargo.toml in $PWD -- \`cavespec\` shim disabled" >&2
            fi

            cavespec_hooks="$(git rev-parse --git-path hooks 2>/dev/null || true)"
            if [ -n "$cavespec_hooks" ] && [ -d "$cavespec_hooks" ]; then
              install -m 755 ${gitHook pkgs "pre-commit"} "$cavespec_hooks/pre-commit"
              install -m 755 ${gitHook pkgs "pre-push"} "$cavespec_hooks/pre-push"
            else
              echo "cavespec(shell): no git hooks directory -- hooks not installed, CI is the gate" >&2
            fi
            unset cavespec_hooks
          '';
        };
      });
    };
