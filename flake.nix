{
  ...
  outputs =
    ...
    let
      ...
      gitHook =
        pkgs: name:
        pkgs.writeText "nanokit-${name}-hook" ''
          #!/bin/sh
          # Generated by nanokit's dev shell (flake.nix), rewritten on every
      ...
      devShells = forAll (pkgs: {
        default = pkgs.mkShell {
          ...
          shellHook = ''
            ...
            nanokit_hooks="$(git rev-parse --git-path hooks 2>/dev/null || true)"
            if [ -n "$nanokit_hooks" ] && [ -d "$nanokit_hooks" ]; then
              install -m 755 ${gitHook pkgs "pre-commit"} "$nanokit_hooks/pre-commit"
              install -m 755 ${gitHook pkgs "pre-push"} "$nanokit_hooks/pre-push"
            else
              echo "nanokit(shell): no git hooks directory -- hooks not installed, CI is the gate" >&2
            fi
            unset nanokit_hooks
          '';
        };
