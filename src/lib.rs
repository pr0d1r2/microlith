...
//! consumer embeds them instead of re-porting them (V6). That is the whole
...
//! milestone.
//!
//! CPU only -- no inference, no network (V5). Every operation is a
...
impl Output {
    ...
    /// A violation the caller asked us to gate on: exit 1 (V9).
    #[must_use]
...
/// V4: an over-long line names its place, so the reader can go there.
...
mod tests {
    ...
    /// V4: an over-long line is refused with its number and its length,
