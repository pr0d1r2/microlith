...
T6|~|the gate. `flake.nix`+`.envrc` DONE; `pkl/Config.pkl` vendored VERBATIM (exempt from every formatter -- reformatting vendored bytes forks them); `hk.pkl` and `ci.yml` are on disk as ANNOTATED DRAFTS, inert by construction (no hooks installed, workflow on `workflow_dispatch` only) ∴ nothing runs a gate that has not been verified. REMAINING: run `hk check --all`, fix what it finds, re-measure coverage AFTER the gate exists and set the floor from THAT number (94.68% today ∴ floor 94, V21), install hooks in the shellHook BY HAND (V23), flip `ci.yml` to `on: push/pull_request`, add `actionlint` to the shell once a workflow really exists. DROPPED as consumer-shaped, ⊥ forgotten: the cassette/network axis · `--no-default-features` (no features here) · `package` (nothing published until T8) · `shellcheck` (no shell scripts -- a linter with nothing to lint is a rule with no runner) · `fetch-depth: 0` (no test reads `HEAD~n`). Verify: `hk check --all` exit 0 · `nix build` · a fresh shell installs the hooks · a missing runner SKIPS loudly, ⊥ blocks|V21,V22,V23,V10
